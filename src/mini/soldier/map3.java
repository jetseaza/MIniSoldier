...
public class map3 extends JPanel implements ActionListener{
    ...
    private ImageIcon background = new ImageIcon(this.getClass().getResource("city2.png"));
    ...
    public ArrayList<badguy4> mybad4 = new ArrayList<badguy4>();
    ...
    Thread timebad4 = new Thread(new Runnable(){
        public void run(){
            while(true){
                try{                          
                    if(checkstart == true){
                        mybad4.add(new badguy4());
         ...
         public void run() {
             while(true){
                try{
                    ...
                    for(int i=0; i<mybad4.size(); i++){
                        mychar.distanceP = (int)Math.sqrt((Math.pow(Math.abs(mychar.x-mybad4.get(i).getX()),2))+(Math.pow(Math.abs(mychar.y-mybad4.get(i).getY()),2)));
                        if(mychar.distanceP <= 80){
                            checknearsrus = 1;
                            mychar.alive -= 5;
                        }
    ...
    public void paintComponent(Graphics g){
        ...
        for(int i=0 ; i<mybad4.size();i++){
            g.drawImage(mybad4.get(i).getImage() ,mybad4.get(i).getX(),mybad4.get(i).getY(),100,100,this);
        }
        //badright
        if(mychar.checkshoot == 1){
            ...
            for(int i=0; i<bullet.size(); i++){
                for(int j=0; j<mybad4.size(); j++){
                    if(Intersect(bullet.get(i).getbound(),mybad4.get(j).getbound())){
                        bullet.remove(i);
                        mybad4.remove(j);
                        score += 10;
                        mychar.checkshoot = 0;
